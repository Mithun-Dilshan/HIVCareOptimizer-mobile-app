...
const DetailedAnalysisScreen = ({ navigation, route }) => {
  ...
  const drugClasses = [
    { 
      name: 'PI', 
      mutations: patientData?.PI_MU_Count || 0,
      resistance: Math.min(((patientData?.PI_MU_Count || 0) / 10) * 100, 100),
      level: (patientData?.PI_MU_Count || 0) >= 7 ? 'High' : (patientData?.PI_MU_Count || 0) >= 4 ? 'Medium' : 'Low'
    },
    { 
      name: 'NRTI', 
      mutations: patientData?.NRTI_MU_Count || 0,
      resistance: Math.min(((patientData?.NRTI_MU_Count || 0) / 10) * 100, 100),
      level: (patientData?.NRTI_MU_Count || 0) >= 7 ? 'High' : (patientData?.NRTI_MU_Count || 0) >= 4 ? 'Medium' : 'Low'
    },
    { 
      name: 'NNRTI', 
      mutations: patientData?.NNRTI_MU_Count || 0,
      resistance: Math.min(((patientData?.NNRTI_MU_Count || 0) / 10) * 100, 100),
      level: (patientData?.NNRTI_MU_Count || 0) >= 7 ? 'High' : (patientData?.NNRTI_MU_Count || 0) >= 4 ? 'Medium' : 'Low'
    },
  ];
  ...
  const getDrugStatus = (drugName, drugClass) => {
    const classData = drugClasses.find(dc => dc.name === drugClass);
    if (!classData) return { status: 'Unknown', icon: 'help-circle-outline', color: COLORS.textSecondary };

    if (classData.level === 'High') {
      return { status: 'High Resistance', icon: 'alert-circle', color: COLORS.danger };
    } else if (classData.level === 'Medium') {
      return { status: 'Intermediate', icon: 'warning', color: COLORS.warning };
    } else {
      return { status: 'Susceptible', icon: 'checkmark-circle', color: COLORS.success };
    }
  };
  ...
  const generateKeyDrivers = () => {
    const drivers = [];
    const totalMutations = apiResult?.total_mutations || 0;
    
    if ((patientData?.NRTI_MU_Count || 0) >= 4) {
      drivers.push(`High NRTI mutations (${patientData?.NRTI_MU_Count || 0}) indicating resistance to nucleoside drugs`);
    }
    if ((patientData?.PI_MU_Count || 0) >= 4) {
      drivers.push(`Elevated PI mutations (${patientData?.PI_MU_Count || 0}) affecting protease inhibitor efficacy`);
    }
    if ((patientData?.NNRTI_MU_Count || 0) >= 4) {
      drivers.push(`NNRTI mutations (${patientData?.NNRTI_MU_Count || 0}) limiting non-nucleoside options`);
    }
    if ((patientData?.viralLoad || 0) > 50000) {
      drivers.push(`High viral load (${parseInt(patientData?.viralLoad || 0).toLocaleString()} copies/mL) indicates active viral replication`);
    }
    if ((patientData?.cd4Count || 0) < 200) {
      drivers.push(`Low CD4+ count (${patientData?.cd4Count || 0} cells/μL) suggests advanced immunosuppression`);
    }
    if (totalMutations >= 8) {
      drivers.push(`Total mutation count (${totalMutations}) exceeds threshold for treatment modification`);
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.detailContainer}>
        ...
        {individualDrugs.map((drug, index) => (
          <View key={index} style={styles.drugStatusRow}>
            <Icon name={drug.icon} size={24} color={drug.color} style={styles.drugStatusIcon} />
            <View style={styles.drugInfo}>
              <Text style={styles.drugStatusName}>{drug.name}</Text>
              <Text style={styles.drugClass}>({drug.drugClass} class)</Text>
            </View>
        ...
        <View style={styles.keyDriversBox}>
          {keyDrivers.map((driver, index) => (
            <View key={index} style={styles.keyDriverItemContainer}>
              <Icon name="chevron-forward" size={16} color={COLORS.primary} style={styles.bulletIcon} />
              <Text style={styles.keyDriverItem}>
                {driver}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.probabilityBox}>
          {apiResult?.all_probabilities ? Object.entries(apiResult.all_probabilities)
            .sort((a, b) => b[1] - a[1]) // Sort by probability descending
            .map(([level, prob]) => (
              <View key={level} style={styles.probDetailRow}>
                <Text style={styles.probDetailLevel}>{level}:</Text>
        ...
        >
          <Icon name="trending-up-outline" size={20} color={COLORS.white} style={{ marginRight: 8 }} />
          <Text style={styles.actionButtonText}>Show SHAP Analysis</Text>
        </TouchableOpacity>
